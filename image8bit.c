...
{ ///
  ...
  if (img == NULL)
  {
    errCause = "Memory allocation failed on the creation of the Image object";
    ImageErrMsg();
    return NULL;
  }
  img->width = width;
  ...
  if (img->pixel == NULL)
  {
    errCause = "Memory allocation failed on the creation of the Image object's pixels ";
    ImageErrMsg();
    free(img->pixel);
    free(img);
    return NULL;
...
void ImagePrint(Image img)
{
  ...
  printf("Image maxval: %c\n", img->maxval);
  printf("Image pixel: %hhn\n", img->pixel);
}
/// Pixel stats
...
{ ///
  assert(img != NULL);
  assert(0 <= level && level <= img->maxval);
  assert(ImageValidPos(img, x, y));
...
{ ///
  ...
  for (int x = 0; x < width; x++)
  {
    ...
    {
      double f = ImageGetPixel(img, x, y) * factor;
      double p = f <= ImageMaxval(img) ? f : ImageMaxval(img);
      ImageSetPixel(img, x, y, p + .5);
    }
  }
}
...
{ ///
  ...
  for (int x = 0; x < width; x++)
  {
    for (int y = 0; y < height; y++)
    {
      ...
      ImageSetPixel(img2, y, height - x - 1, ImageGetPixel(img, x, y));
    }
  }
  if (img2 == NULL)
...
{ ///
  ...
  for (int i = 0; i < w; i++)
  {
    for (int j = 0; j < h; j++)
    {
      double v = ImageGetPixel(img1, i + x, j + y) * (1 - alpha) + ImageGetPixel(img2, i, j) * alpha;
      ImageSetPixel(img1, i + x, j + y, v + .5);
    }
  }
}
...
/// 2ª versão
// Optimized version
//comentar e descomentar mediante os testes de complexidade (testBlur) 
